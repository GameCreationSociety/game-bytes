...
namespace Tetris_2p
{
    public class Game_2 : MonoBehaviour
    {
        ...
        {
            FindObjectOfType<Game_1>().IncreaseAboveRows();
            for (int i = 0; i < 10; i++)
            {
                if (i != x)
                {
                    var clone = Instantiate(Holes, FindObjectOfType<Game_1>().transform.position
                                                   + (float)i / 2 * Vector3.right, Quaternion.identity);
                    FindObjectOfType<Game_1>().UpdateCheese(clone, i, 0);
                }
            }
        }
        ...
        public Tetromino_2 GetHoldTetromino()
        {
            return t;
        }
